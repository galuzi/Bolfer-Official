...
{
    private const RESET_CODE_ALPHABET = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
    ...
    public function issueForUser(array $user): array
    {
        ...
        if ($userId <= 0 || $email === '') {
            return [
                'ok' => false,
                'delivery' => 'none',
                'message' => 'Conta inválida para recuperação de senha.',
            ];
        }

        $plainToken = $this->generateResetCode();
        $tokenHash = hash('sha256', $plainToken);
        $this->users->setPasswordResetToken($userId, $tokenHash);
    ...
    public function validateToken(string $plainToken): array
    {
        ...
        if ($candidates === []) {
            return [
                'ok' => false,
                'reason' => 'invalid',
                'message' => 'Código ou link de recuperação inválido.',
            ];
        }

        $user = null;
        foreach ($candidates as $candidate) {
            $user = $this->users->findByPasswordResetTokenHash(hash('sha256', $candidate));
            if ($user) {
                break;
            }
        }
        ...
        if (!$user) {
            return [
                'ok' => false,
                'reason' => 'invalid',
                'message' => 'Código ou link de recuperação inválido ou já utilizado.',
            ];
        }

        if (!$this->isWithinTtl((string) ($user['password_reset_sent_at'] ?? ''))) {
            return [
                'ok' => false,
                'reason' => 'expired',
                'user' => $user,
                'message' => 'Este link de recuperação expirou. Solicite um novo envio.',
            ];
        }
        ...
        return [
            'ok' => true,
            'user' => $user,
            'message' => 'Código validado. Agora é só escolher a nova senha.',
        ];
    }

    public function resetPassword(string $plainToken, string $newPassword): array
    {
        $validation = $this->validateToken($plainToken);
        if (empty($validation['ok'])) {
            return $validation;
        }

        $user = (array) ($validation['user'] ?? []);
        $userId = (int) ($user['id'] ?? 0);
        if ($userId <= 0) {
            return [
                'ok' => false,
                'reason' => 'invalid',
                'message' => 'Não foi possível localizar a conta para redefinir a senha.',
            ];
        }

        $this->users->updatePassword($userId, password_hash($newPassword, PASSWORD_DEFAULT));
        $updatedUser = $this->users->findById($userId) ?? $user;

        return [
            'ok' => true,
            'user' => $updatedUser,
            'message' => 'Senha redefinida com sucesso. Agora você já pode entrar na sua conta.',
        ];
    }

    public function ttlMinutes(): int
    {
        return max(10, (int) \env('USER_PASSWORD_RESET_TTL_MINUTES', '60'));
    }
    ...
    private function logDelivery(string $email, string $username, string $plainToken): array
    {
        ...
        if ($written === false) {
            return [
                'ok' => false,
                'delivery' => 'log',
                'message' => 'Não foi possível registrar a recuperação de senha localmente.',
            ];
        }
        ...
        return [
            'ok' => true,
            'delivery' => 'log',
            'message' => 'Link de recuperação salvo no ambiente local.',
    ...
    private function smtpDelivery(string $email, string $username, string $plainToken): array
    {
        try {
            ...
            return [
                'ok' => false,
                'delivery' => 'smtp',
                'message' => 'Não foi possível enviar o e-mail de recuperação. Revise o SMTP da conta no-reply@example.com.',
            ];
        }
    }
    ...
    {
        $payload = $this->buildMessagePayload($email, $username, $plainToken);
        $boundary = 'bolfer-reset-' . bin2hex(random_bytes(12));
    ...
    private function buildMessagePayload(string $email, string $username, string $plainToken): array
    {
        ...
        $subject = 'Redefina sua senha - Bolfer Official';
        $resetUrl = $this->resetUrl($plainToken);
        ...
        $htmlMessage = '<!doctype html><html><body style="margin:0;padding:32px;background:#080706;color:#f5eee3;font-family:Arial,sans-serif;">'
            ...
            . '<p style="margin:0 0 10px;color:#96aa8d;font-size:12px;letter-spacing:3px;text-transform:uppercase;">Recuperação da conta</p>'
            . '<h1 style="margin:0 0 16px;font-size:36px;line-height:1.05;color:#fff4e8;text-transform:uppercase;">Redefina sua senha com segurança</h1>'
            ...
            . '<p style="margin:0 0 24px;font-size:16px;line-height:1.7;color:#e7dccd;">Recebemos um pedido para redefinir a senha da sua conta. Use o botão abaixo para continuar ou copie o código de recuperação se preferir preencher manualmente na tela do site.</p>'
            . '<div style="margin:0 0 24px;padding:22px;border-radius:20px;background:#121614;border:1px solid rgba(57,255,20,0.18);">'
            . '<p style="margin:0 0 10px;font-size:12px;letter-spacing:2px;text-transform:uppercase;color:#96aa8d;">Código de recuperação</p>'
            ...
            . '<p style="margin:12px 0 0;text-align:center;font-size:13px;color:#cdbfae;">Este código expira em ' . $this->ttlMinutes() . ' minuto(s).</p>'
            . '</div>'
            . '<div style="margin:0 0 26px;text-align:center;">'
            . '<a href="' . htmlspecialchars($resetUrl, ENT_QUOTES, 'UTF-8') . '" style="display:inline-block;padding:16px 28px;border-radius:14px;background:#39ff14;color:#071006;text-decoration:none;font-weight:700;letter-spacing:1px;text-transform:uppercase;">Criar nova senha</a>'
            . '</div>'
            ...
            . '<p style="margin:0;font-size:14px;line-height:1.7;word-break:break-all;"><a href="' . htmlspecialchars($resetUrl, ENT_QUOTES, 'UTF-8') . '" style="color:#8fd3ff;text-decoration:none;">' . htmlspecialchars($resetUrl, ENT_QUOTES, 'UTF-8') . '</a></p>'
            . '</div>'
            . '<p style="margin:0 0 10px;font-size:14px;line-height:1.7;color:#cdbfae;">Se você não pediu a redefinição da senha, ignore esta mensagem. Nenhuma alteração será concluída sem a confirmação pelo link ou pelo código.</p>'
            . '</div>'
        ...
        $textMessage = implode("\n", [
            ...
            '',
            'Código de recuperação: ' . $displayCode,
            'Este código expira em ' . $this->ttlMinutes() . ' minuto(s).',
            '',
    ...
    private function generateResetCode(int $length = 12): string
    {
        $length = max(8, $length);
        $code = '';
        $maxIndex = strlen(self::RESET_CODE_ALPHABET) - 1;

        for ($i = 0; $i < $length; $i++) {
            $code .= self::RESET_CODE_ALPHABET[random_int(0, $maxIndex)];
        }
    ...
    private function resetUrl(string $plainToken): string
    {
        return \url('/forgot-password/reset?token=' . rawurlencode($plainToken));
    }
    ...
    {
        return dirname(__DIR__, 2) . '/storage/logs/password_reset_mail.log';
    }
}
